...
@section('description')
ข้อมูลการสั่งซื้อของออเดอร์ <?php echo $order->id; ?>
@endsection
      ...
      <?php if($order->status->id==1){ ?>
        <div class="row">
          <div class="alert alert-info w-100">
            ...
            <a href="/order-payment/<?php echo $order->id; ?>/<?php echo $order->order_hash; ?>/" class="btn btn-primary mt-2" role="button">ไปชำระเงิน</a>&nbsp;<a href="/dashboard/order/cancel/<?php echo $order->id ?>/<?php echo $order->order_hash ?>/" class="btn mt-2 small text-muted">ยกเลิกการสั่งซื้อ</a>
          </div>
          </div>
        ...
        <div class="row">
          <div class="alert alert-success w-100">
            <?php if(!$order->reviews->isEmpty()){ ?>
              ...
              <a href="/guest-order/<?php echo $order->id; ?>/<?php echo $order->order_hash; ?>/review/"  class="btn btn-primary mt-2" id="review-button" role="button">ดูรีวิวที่คุณเขียน</a>
           <?php }else{ ?>
              ออเดอร์นี้สำเร็จเป็นที่เรียบร้อยแล้ว ได้โปรดให้คะแนนและเขียนข้อความสั้นๆ เพื่อปรับปรุงคุณภาพและบริการของร้านค้า<br>
              <a href="/guest-order/<?php echo $order->id; ?>/<?php echo $order->order_hash; ?>/review/"  class="btn btn-warning mt-2" id="review-button" role="button">เขียนรีวิว</a>
           <?php } ?>
